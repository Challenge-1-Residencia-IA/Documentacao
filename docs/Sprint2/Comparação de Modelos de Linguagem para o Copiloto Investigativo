...
Para escolher o melhor modelo para o projeto, não basta olhar para benchmarks genéricos. É preciso avaliar o que o sistema realmente faz: buscar evidências e gerar uma análise baseada nelas. Por isso, as métricas mais importantes são as do framework RAGAS (Retrieval-Augmented Generation Assessment) .

Métricas Principais

Métrica	O Que Mede	Por Que é Importante
Faithfulness (Fidelidade)	Mede se a análise gerada está baseada exclusivamente nas evidências recuperadas, sem inventar informações 	É a métrica mais crítica para o projeto. O sistema não pode alucinar.
